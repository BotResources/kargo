...
func TestWatchFreight_resourceVersion(t *testing.T) {
	t.Parallel()
	...
	stream, err := cli.WatchFreight(ctx, connect.NewRequest(&svcv1alpha1.WatchFreightRequest{
		Project:         projectName,
		ResourceVersion: "123",
	}))
	require.NoError(t, err)
	require.True(t, stream.Receive())
	require.Equal(t, "freight-1", stream.Msg().GetFreight().GetName())
...
func TestWatchFreight_expiredResourceVersionOnStart(t *testing.T) {
	t.Parallel()
...
func TestWatchFreight_filteredModifiedEventSendsDelete(t *testing.T) {
	t.Parallel()
